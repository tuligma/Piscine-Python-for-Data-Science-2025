...
class Research:
    ...
    def file_reader(self, has_header=True) -> list:
        ...
        try:
            ...
            logging.debug(f"Reading the file from {self.path}")
    ...
    @staticmethod
    def send_notification(reportfile):
        success_msg = 'The report has been successfully created'
        failure_msg = 'The report hasn’t been created due to an error'
        webhook_url = os.getenv("SLACK_WEBHOOK_URL")
        if not webhook_url or not webhook_url.startswith("https://hooks.slack.com"):
            raise ValueError("send_notification(): SLACK_WEBHOOK_URL env is not set.")
        message = {}
        ...
        try:
            ...
            print(
                f"Status: {response.status_code}|{response.text.upper()} "
                f"-> Message Sent : {message['text']}"
            )
        except requests.exceptions.RequestException:
            raise RuntimeError(
                "send_notification(): Status: "
                f"{response.status_code}|{response.text.upper()} -> "
                f": {response.reason}"
            )
...
class Analytics(Research.Calculations):
    ...
    def predict_random(self, num):
        ...
        """
        logging.debug("Making a prediction of head and tails")
    ...
    def predict_last(self):
        ...
        """
        logging.debug("Showing the last item on the list of head and tails")
    ...
    def save_file(data, filename, ext='txt'):
        ...
        """
        logging.debug(f"Saving the report to '{filename}.{ext}'")
        try:
